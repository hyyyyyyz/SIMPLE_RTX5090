...
_PLACE_HEIGHT = 0.45
_LOWER_HEIGHT = 0.05
@TaskRegistry.register("g1_wholebody_bend_handover_teleop")
class G1WholebodyBendHandoverTeleop(Task):
    uid:str = "g1_wholebody_bend_handover_teleop"
    label: str = "G1 Wholebody Bend Handover teleop"
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        spatial = SpatialDRCfg(
            ...
            target_region=Box(low=[-0.35 ,-0.08], high=[-0.4, -0.04]),
            container_region=Box(low=[-0.2, 0.15], high=[-0.25, 0.25]),
            distractors_region=Box(low=[-0.1, -0.3], high=[0.4, 0.3]),
