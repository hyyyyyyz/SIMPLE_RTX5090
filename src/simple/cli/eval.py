...
def _run_eval_worker(
    ...
    report(
        ...
        global_total=len(global_episode_indices),
        # Without this the TUI has no denominator and the STEP column shows a bare
        # step count. Resolved above, so it also covers the task-metadata fallback
        # that applies when --max-episode-steps is left unset.
